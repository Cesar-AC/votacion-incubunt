...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 text-gray-800">Crear Candidato</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form id="createCandidatoForm" action="{{ route('crud.candidato.crear') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="idUsuario" name="idUsuario" required>
                        ...
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->idUser }}">{{ $usuario->correo }}</option>
                        @endforeach
                    </select>
                </div>
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save"></i> Crear Candidato
                </button>
            </form>
        </div>
    </div>
...
<script>
document.getElementById('createCandidatoForm').addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    .catch(error => {
        ...
        alert('Ocurrió un error al crear el candidato');
    });
});
</script>
@endsection
