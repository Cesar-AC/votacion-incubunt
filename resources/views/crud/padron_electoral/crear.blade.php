    ...
    <div class="card shadow-sm mb-3">
      <div class="card-body py-3">
        <div class="form-group mb-0">
          <label class="small font-weight-bold">Elecciones disponibles</label>
          <select name="idElecciones" class="form-control" required>
            ...
            <option value="{{ $e->idElecciones }}">
              {{ $e->titulo }} ({{ \Carbon\Carbon::parse($e->fechaInicio)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($e->fechaCierre)->format('d/m/Y') }})
            </option>
            @endforeach
          </select>
        </div>
